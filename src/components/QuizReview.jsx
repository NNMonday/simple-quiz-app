...
export default function QuizReview(props) {
    const { currentQ, qList, userChoice } = props
            ...
            <div className='quiz-answer w-100 pt-5' style={{ backgroundColor: '#a5b4fb' }}>
                {quiz.answers.map((answer, indexA) => (
                    <div className='w-100 ' key={indexA}>
                        <p
                            className={`shadow-sm rounded-3 answer ${qList[currentQ].answers[indexA].correct ? 'correct' : (userChoice[currentQ].answer === indexA && !userChoice[currentQ].correct ? 'wrong' : '')}`}
